...
The likelihood equation is defined in the package, and a wrapper in the package calls R's built in `optim` function to maximize the likelihood of the parameters within each strata.


```{r}
#the control option is passed to optim, telling it to run up to 5000 iterations
mlePointEstimates <- MLEwrapper(multStratData, tags = tags, GSIcol = "GSI", PBTcol = "GenParentHatchery", strataCol = "Strata", adFinCol = "AdClip", AI = TRUE, optimMethod = "Nelder-Mead", variableCols = NULL, control = list(maxit=5000))
```
...
To get confidence intervals for these estimates, we can use non-parametric bootstrapping. This is computationally intensive and the code is not particularly well optimized, so it may take a while depending on how much data you have. We will set up a simple `for` loop to perform the bootstrapping, and we will save the estiamtes from each iteration. In this example, we are only interested in `piTot`, but if you were intereted in the other estimates, you would want to save them as well. We are also only going to generate confidence intervals for all strata combined, but you could save the results for each strata separately if you wanted a confidence interval for each strata.
...
for(b in 1:bootRep){
	...
	bootEst <- MLEwrapper(bootData, tags = tags, GSIcol = "GSI", PBTcol = "GenParentHatchery", strataCol = "Strata", adFinCol = "AdClip", AI = TRUE, optimMethod = "Nelder-Mead", variableCols = NULL, control = list(maxit=5000))
