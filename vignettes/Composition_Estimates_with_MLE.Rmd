...
## going from proportions to numbers
...
To get confidence intervals for these estimates, we can use non-parametric bootstrapping. This is computationally intensive, so it may take a while depending on how much data you have. We will set up a simple `for` loop to perform the bootstrapping, and we will save the estiamtes from each iteration. In this example, we are only interested in `piTot`, but if you were intereted in the other estimates, you would want to save them as well.
